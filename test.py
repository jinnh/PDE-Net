...
from model.pde_net import reconnetHRHSI as HSIG
    ...
    with torch.no_grad():
        ...
        for index in range(len(images_name)):
            ...
            for i in range(samplingTimes):
                output,_ = model(input, 100)
                ASR.append(output)
            ASR = torch.cat(ASR, dim=0)
            ALLSR = ASR.cpu().numpy().astype(np.float32)
            SR = np.mean(ALLSR, axis=0)
            SR[SR < 0] = 0
            SR[SR > 1.] = 1.
            ...
            rmse = compare_rmse(x_true=HR, x_pred=SR)
            sam = cal_sam(SR, HR)
            ...
            print("===The {}-th picture=====PSNR:{:.4f}=====SSIM:{:.4f}=====SAM:{:.4f}=====RMSE:{:.4f}====Name:{}".format(index + 1, psnr,
                                                                                                         ssim, sam,rmse,
                                                                                                         images_name[index]))
        print("=====averPSNR:{:.4f}=====averSSIM:{:.4f}=====averSAM:{:.4f}=====averRMSE:{:.4f}====".format(np.mean(PSNRs), np.mean(SSIMs),
                                                                           np.mean(SAMs),np.mean(RMSEs)))
