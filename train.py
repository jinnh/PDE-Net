...
if opt.model_name == 'template':
    from model.template import reconnetHRHSI as HSIG
else:
...
def main():
    ...
    if opt.model_name == 'template':
        sys.stdout = Logger(osp.join(save_dir, 'log_train_template.txt'))
    else:
        sys.stdout = Logger(osp.join(save_dir, 'log_train_pde-net.txt'))
    ...
    if opt.resume:
        if os.path.isfile(opt.resume):
            ...
            # opt.start_epoch = param_dict['epoch'] + 1
            opt.start_epoch = 1
    ...
    if opt.model_name == 'template':
        ...
        scheduler = MultiStepLR(optimizer, milestones=[25, 50, 75, 100], gamma=0.5, last_epoch=-1)
